	...
	public function save($items, $customer_id, $employee_id, $comment, $invoice_number, $payments, $sale_id=false)
	{
		...
		foreach($payments as $payment_id=>$payment)
		{
			...
			if( substr($paymentType,0,3)=='Cre' ){
                ...
                $customer_name= $this->Customer->getCustomerName($customer_id) ;
				...
				'amount_tendered'=>  $payment['payment_amount'],
				);

				/*
				 * Now update the credit sales to customer account
				 */
                $customer_bill= $this->Customer->billCustomer($customer_id, $payment['payment_amount']) ;
                ...
                $this->db->insert('credit_transactions',$creditData);
			}
			else{
                $sales_payments_data = array(
                    'sale_id'		=> $sale_id,
