...
class Sales extends Secure_area
{
	...
	function sendAfterSale( $message)
	{
   ...
   $result=  "Message not sent";
	...
	{		
		...
		else
		{
			$payment_amount = $this->input->post('amount_tendered');
                        ...
                        //$sms=new Messages();
                        $message="Hi madam sales successfully made to client worth GHc$payment_amount";
                        @$this->sendAfterSale($message);
		}
	...
	function complete()
	{
		...
		else 
		{
			...
			//$data['sale_id'] = 'POS ' . $this->Credit_Sales->save($data['cart'], $customer_id, $employee_id, $data['comments'], $invoice_number, $data['payments']);
	...
	{	
		...
		if($customer_id != -1)
		{
            ...
            $data['customer_paid'] = $cust_info->paid;
            $data['customer_owing'] = $cust_info->owing;
